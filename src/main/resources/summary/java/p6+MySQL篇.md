...
- **性能**：MySQL 8.0的速度要比MySQL 5.7 快2倍。

- **NoSQL**：MySQL从5.7版本开始提供NoSQL存储功能，在8.0版本中nosql得到了更大的改进。

- **窗口函数**：实现若干新的查询方式。窗口函数与 SUM()、COUNT() 这种集合函数类似，但它不会将多行查询结果合并为一行，而是将结果放回多行当中，即窗口函数不需要GROUP BY。

- **隐藏索引**：在 MySQL 8.0 中，索引可以被“隐藏”和“显示”，当对索引进行隐藏时，不会被查询优化器所使用，可以利用这个特性来进行性能测试，比如先隐藏一个索引，然后观察其对数据库的影响，如果数据库性能有所下降，说明这个索引是有用的，然后将其恢复显示即可；如果数据库性能看不出变化，说明这个索引是多余的，则可以考虑删掉。
...
- **原⼦性**： Atomicity， 事务是最小的执行单位，不允许分割，整个事务中所有的操作，要么全部提交成功，要么全部失败回滚，强调的是**事务操作原子不可分割**。
  - 实现：使用undo log保证原子性。
- **一致性**：Consistency， 数据库总是从一个一致性的状态转换到另外一个一致性的状态，这种状态指的数据处于一种语义上的有意义且正确的状态，比如数据库只包含全部成功后的数据就是数据的一致性，强调的是**数据状态事务前后的一致**。
  - 实现：使用redo log保证一致性。
...
### 2.3. MySQL OPTIMIZER_TRACE优化器跟踪？
