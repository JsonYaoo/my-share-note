...
- **空参的构造方法**：使用默认的负载因子0.75f。
- **指定容量的构造方法**：使用默认的负载因子0.75f。
- **指定容量和负载因子的构造方法**：赋值负载因子和计算2^n作为当前容量。
- **指定复制集合的构造方法**：使用默认的负载因子0.75f，当前容量为复制集合元素大小的2^n。
...
- **抽象的HashMap#HashIterator**：HashMap的普通结点迭代器，快速失败机制，遍历next指针，返回Node对象。
- **HashMap#Entry迭代器**：继承抽象的HashMap#HashIterator，快速失败机制，遍历next指针，返回Map.Entry对象。
- **HashMap#Key迭代器**：继承HashMap#Entry迭代器，快速失败机制，遍历next指针，返回Map.Entry#key对象。
- **HashMap#Value迭代器**：继承HashMap#Entry迭代器，快速失败机制，遍历next指针，返回Map.Entry#Value对象。
...
- **tableSizeFor（int）：**返回给定目标容量的2的幂次作为阈值。
  - **右移的目的**：是为了取得每位都是1的结果（最高位为当前容量的最高位），从而可以通过右移结果+1后可以获得2的幂次容量。

```java
// 返回给定目标容量的2的幂次作为阈值 => 根据容量获取实际容量2^n
...
- **性质4**：红黑树的**红色结点必须有两个黑色结点**，也就是说从根结点到每个叶子结点的所有路径上，不可能存在两个连续的红色结点。
...
- **balanceInsertion（TreeNode，TreeNode）**：对应2-3-4树的情况：
  ...
  - **d. 合并到4结点中**：成为一个裂变状态（变色后相当于升元了），插入前为4结点（黑红红），插入后4结点颜色反转，爷结点成为新的x结点，准备下一轮的向上调整，根据x插入的位置不同分为4种情况：
    ...
    - **d.2.3.** 中左 左*右(黑红 红红)，不符合红黑树定义 => 需要调整，则中变红，左1变黑，左2保持为红，右1变黑，中看作为“插入结点”，继续向上调整。
    - **d.2.4.** 中左 左右*(黑红 红红)，不符合红黑树定义 => 需要调整，则中变红，左1变黑，右1变黑，右2保持为红，中看作为“插入结点”，继续向上调整。
...
- **balanceDeletion（TreeNode，TreeNode）**：对应2-3-4树的情况，x为待删除的结点（替代结点，肯定为叶子结点）：
  - **x自己搞得定**：
    - 自己搞得定的意思就是，可以**在自己结点内部处理完毕**（对应2-3-4树结构），不影响其他树的结构。
    - **a.1. x为3结点或者4结点的红结点**：直接置黑返回x结点即可调整完毕，交由上层方法删除x结点。
  - **x自己搞不定，兄弟搞得定**：
    ...
    - **b.1. 兄弟结点为3结点，但无右（左）**：在x在左子树一方时，x的兄弟结点xpr为右子树，如果xpr无右孩子在对父结点左旋时，会导致xpr为null，导致2-3-4树的结构不正确，因此，**b.1是一个临时情况，需要转换为b.2有右进一步处理**。x为右子树一方时则相反。
  ...
  - **x自己搞不定，而且兄弟也搞不定**：
    ...
    - **c.1. 兄弟结点为2结点**：此时，为了让x能够顺利删除，**兄弟结点需要置红（自损）**，这样x在删除后，x父结点所在树还是黑色平衡的。但是，如果x父结点为黑色，x爷结点所在树则不黑色平衡了（因为父结点这边少了一个黑色结点），所以父结点的叔结点要也要被置红。**因此需要一路向上自损，直到碰到任意一个终止条件即可结束**：
      - **自损的终止条件1（向上碰到根结点）**：经过一路置红叔结点，直到循环到根结点时（因为上面已经没有父节点了），则代表自损完毕，此时整棵树都是黑色平衡的了（都减少了一个黑色结点）。
      - **自损的终止条件2（向上碰到红结点）**：如果碰到红色结点时，只需要把该结点置黑，则不需要在置红叔结点了，此时相当于在父结点这边子树补回了一个黑色结点，而不影响叔结点那边子树的黑色结点数目，因此整棵树还是黑色平衡的。

```java
// 删除结点前/后平衡红黑树，如果x所在结点为2-3-4树的2结点，则平衡后再删除，否则在平衡前对应的结点就已经删除了（此时x作为该结点的替代结点而保留下来）
...
- **抽象的LinkedHashMap#LinkedHashIterator**：LinkedHashMap的普通结点迭代器，快速失败机制，遍历next指针，返回LinkedHashMap#Entry对象。
- **LinkedHashMap#Entry迭代器**：继承抽象的LinkedHashMap#LinkedHashIterator，快速失败机制，遍历next指针，返回Map.Entry对象。
- **LinkedHashMap#Key迭代器**：继承LinkedHashMap#Entry迭代器，快速失败机制，遍历next指针，返回Map.Entry#key对象。
- **LinkedHashMap#Value迭代器**：继承LinkedHashMap#Entry迭代器，快速失败机制，遍历next指针，返回Map.Entry#Value对象。
...
- **afterNodeAccess（Node）**：访问结点后回调：移动结点到链表末尾，代表为最近访问。访问顺序为true，代表访问后会移动元素到链表末尾，插入顺序为false，代表不作任何移动，保持为插入顺序。
