...
abstract static class Sync extends AbstractQueuedSynchronizer {
    ...
    // ThreadLocal本体，命中率1%~5%，每个线程持有的读锁小计数器, 每当线程的读取保留计数降至0时删除
    private transient ThreadLocalHoldCounter readHolds;
    
    // 二级缓存，命中率5%~10%，最近一个获取读锁线程的读锁小计数器, 可以节省ThreadLocal查找, 非常适合线程缓存
    private transient HoldCounter cachedHoldCounter;
    
    // 一级缓存读锁线程，命中率90%~95%，是第一个获得读锁的线程, 它是最后一次将共享计数从0更改为1的唯一线程, 此后再没有释放读锁
    private transient Thread firstReader = null;

    // 一级缓存读锁小计数值，命中率90%~95%，firstReader的保留计数, 允许跟踪无竞争读锁的读保持非常便宜
...
protected final boolean tryReleaseShared(int unused) {
    ...
    // 如果当前线程为第一个读线程, 说明命中了一级缓存
    if (firstReader == current) {
        // assert firstReaderHoldCount > 0;
        // 如果一级缓存计数器值为1，则清空一级缓存
        if (firstReaderHoldCount == 1)
            firstReader = null;
        // 如果一级缓存计数值不为1，则一级缓存的值-1
        else
            firstReaderHoldCount--;
    }
    // 如果当前线程不为第一个读取锁线程, 说明没命中一级缓存，则需要尝试获取二级缓存
    else {
        ...
        // 如果计数值小于等于1，说明在释放读锁后，当前ThreadLocal本体中的二级缓存会为0，则先从本体中移除当前线程的二级缓存
        if (count <= 1) {
            readHolds.remove();
            if (count <= 0)
...
protected final int tryAcquireShared(int unused) {
    ...
    if (!readerShouldBlock() && r < MAX_COUNT && compareAndSetState(c, c + SHARED_UNIT)) {
        ...
        // 如果当前线程为第一个读锁线程, 说明命中一级缓存，则直接在缓存上累加锁持有数量
        ...
        // 如果当前线程不为第一个读取锁线程, 说明没命中一级缓存，则需要尝试获取二级缓存
            ...
            // 如果二级缓存为空，获取二级缓存中的线程id不为当前线程id，即小计数器不对，则使用ThreadLocal本体获取小计数器，作为新的二级缓存
            if (rh == null || rh.tid != getThreadId(current))
                cachedHoldCounter = rh = readHolds.get();
            // 如果二级缓存为当前线程的小计数器，说明命中了二级缓存，则重新设置当前的小计数器
            else if (rh.count == 0)
                readHolds.set(rh);
...
final int fullTryAcquireShared(Thread current) {
    ...
    for (;;) {
        ...
        // 如果读锁应该阻塞, 判断当前线程是否为第一个获取读锁的线程, 如果是则什么也不做, 如果不是则更新缓存, 此时如果缓存计数为0, 则返回-1, 代表读锁获取失败
            ...
            // 如果当前线程为第一个获取读锁线程, 说明命中了一级缓存，则什么也不做
            if (firstReader == current) {
                // assert firstReaderHoldCount > 0;
            }
            // 如果当前线程不为第一个获取读锁线程, 说明没有命中一级缓存
            else {
                // 如果没有命中二级缓存，则初始化二级缓存，如果二级缓存的小计数为0，则需要从ThreadLocal本体中移除，并返回-1，代表读锁获取失败，因为当前线程读锁已经释放掉了
                if (rh == null) {
        ...
        if (compareAndSetState(c, c + SHARED_UNIT)) {
            // 如果CAS成功, 且读锁次数为0，则初始化一级缓存的计数值为1
            if (sharedCount(c) == 0) {
                firstReader = current;
                firstReaderHoldCount = 1;
            } 
            // 如果读锁次数不为0，且命中了一级缓存，则更新一级缓存的计数值+1
            else if (firstReader == current) {
                firstReaderHoldCount++;
            } 
            // 如果当前线程不为第一个读取锁线程, 说明没命中一级缓存，则需要尝试获取二级缓存
