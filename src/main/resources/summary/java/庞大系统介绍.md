...
随机频繁写会移动磁盘，且属于系统调用，需要从用户态切换到内核态。
...
答了type、extra与索引、group by调优、order by调优
...
2. 顺序写盘、
3. PageCache空中接力、
4. mmap零拷贝减少了读取网络数据时的2次用户态数据拷贝，
5. sendfile零拷贝减少了读取磁盘文件时的2次用户态数据拷贝，以及1次socket内核缓冲区到NIC BUFFER的数据拷贝
6. 生产端消息批量发送、
7. 消息压缩机制
...
#### 12.2.1.5. zk 做的分布式锁有什么缺点？

见《[1.2.1.4. 分布式锁的实现方案，以及红锁的缺点？](#1.2.1.4. 分布式锁的实现方案，以及红锁的缺点？)》。

#### 12.2.1.6. zk 分布式锁从 CAP 的角度进行回答？



#### 12.2.1.7. zk 主节点宕机后，还能保证高可用吗？

答了不能，需要花费时间进行日志复制

ZAB：主节点宕机后需要经过两个模式，1. 恢复模式选主，2. 广播模式同步，确定好ZXID同步点，然后就可以继续对外提供服务了

#### 12.2.1.8. zk 主节点选举过程的不可用，有什么手段可以继续优化吗？

答了这是有分布式一致性算法决定的，可能会有调优参数，应该能够减少时间，但不能完全避免
...
#### 12.2.2.1. 算法题 - 数组中，找出任意一个峰值元素的索引？

写了与前一个值比较，O（n），然后优化了两次中间变量，最终不适用任何中间变量，额外空间复杂度为O（1）
=> leetcode162：O(logn)
...
1）答了bitmap不行，散列会冲突
​2）答了大文件分片，次数作为文件夹名称也不行，找不到上一次的次数是啥
​3）答了top k小根堆，应该可以，节点存关键词的值和次数，不断交换堆头出去

#### 12.2.2.3. 小根堆和大根堆的区别？

答了小根堆堆顶是最小的，大根堆堆顶是最大的

#### 12.2.2.4. 小根堆，加入一个节点，是如何做调整的？

答了一般是在末尾添加，这时需要不断地与父节点比较，小的则交换上去，这叫做向上调整

应该还答下不在末尾添加节点时，需要先向下调整，与孩子节点到叶子节点比较，把大的换下去，然后在向上调整，不断地与父节点比较，小的则换上去
